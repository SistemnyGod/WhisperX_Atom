...
сохраняет в отчёт schema v2 хэши вопросов, идентификаторы, expected/actual
режимы и intent, отдельные результаты mode/intent/outcome/answer-type/evidence/
grounding и timings. Любое несовпадение делает case failed и возвращает
ненулевой exit code; ноль реально выполненных случаев также является ошибкой.
Тексты вопросов/ответов, стенограммы и персональные данные в отчёт не
попадают; production QA-корпус хранится локально и не входит в Git или release
bundle. Для детерминированного preflight reasoning используется
