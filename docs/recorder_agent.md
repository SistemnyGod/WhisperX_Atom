...
- в профиле ONLINE параллельно писать `room-microphone` и `system-audio`; при
  недоступности loopback профиль деградирует до MIC_ONLY без потери микрофона;
- пробуждать delivery при reconnect и завершении FLAC, включая progressive upload
  уже закрытых чанков во время продолжения записи.

Исходные аудиофайлы из `C:\Users\AI_server\Documents\Audacity` автоматически
не читаются. Системный loopback является опциональным вторым источником профиля
ONLINE; он не используется для голосовых команд и не блокирует MIC_ONLY.
