...
def _iter_window_rms(path: Path, window_ms: int = 100) -> Iterable[tuple[float, float, float, int]]:
    """Yield RMS, peak, clipping fraction and sample count per bounded window."""
    try:
        import numpy as np
...
def analyze_wav(path: Path, window_ms: int = 100) -> AudioSignalMetrics:
    ...
    clipping: list[float] = []
    total_windows = 0
    for rms, peak, clipped, _ in _iter_window_rms(path, window_ms):
