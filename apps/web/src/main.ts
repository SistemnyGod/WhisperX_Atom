...
type Media = { id: string; originalName: string; sizeBytes: number; durationMs?: number; sha256?: string; status: string; archiveStorageKey?: string; previewStorageKey?: string; asrStorageKey?: string };
...
const app = {
  setup() {
    ...
    function subscribeJobs() {
      ...
      for (const job of jobs.value) {
        ...
        source.onerror = () => {
          source.close();
          window.setTimeout(() => { if (selected.value) refreshSelected().catch(() => undefined); }, 3000);
        };
  ...
  template: `
    <main class="shell">
      ...
      <template v-else>
        <header class="topbar"><div><strong>WhisperX Atom</strong><span>server-first MVP</span></div><div><button class="secondary" @click="loadMeetings">Обновить</button><button class="secondary" @click="logout">Выйти</button></div></header>
        <div class="layout">
