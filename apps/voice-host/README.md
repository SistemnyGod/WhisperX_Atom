...
- To stage verified Shadow assets locally, build `apps\voice-host\native\whisper-refiner` at whisper.cpp `f049fff95a089aa9969deb009cdd4892b3e74916` (the native build accepts `-WhisperCppSource` for an already checked-out pinned source), then run `scripts\stage-voice-refiner-assets.ps1` with the model/native paths, full model and bridge provenance, and the clean build identity. The required multilingual `ggml-small.bin` revision is `c521a4b02f422512d734391fdf08bb08c0862f68` with SHA256 `1be3a9b2063867b937e64e2ec7483364a79917e157fa98c5d94b5c1fffea987b`. The resulting manifest is schema v2 and is verified by `scripts\verify-voice-refiner-assets.ps1`. Release packaging passes `-RequireVoiceRefinerAssets`; it fails closed unless the manifest, ABI and both SHA256 values verify. Development builds may leave the mode `OFF` or run `SHADOW` with an explicit `UNAVAILABLE` snapshot.
