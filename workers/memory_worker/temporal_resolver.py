...
def current_state(facts: Iterable[MemoryFact], relations: Iterable[MemoryRelation] = ()) -> tuple[MemoryFact, ...]:
    ...
    for fact in values:
        subject_key = (fact.subject or "").lower().strip()
        if not subject_key:
    ...
    for group in groups.values():
        ...
        if len(values_by_text) > 1 and len(eligible or group) > 1:
            selected.extend(sorted(eligible or group, key=lambda fact: (fact.meeting_id, fact.start_ms, fact.end_ms)))
        else:
            selected.append(max(eligible or group, key=lambda fact: (fact.meeting_id, fact.start_ms, fact.end_ms)))
    return tuple(sorted(selected, key=lambda fact: (fact.fact_type, fact.subject or "")))
