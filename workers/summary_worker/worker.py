...
from .llama_subprocess import LocalLlamaRuntime, attest_llm_model, resolve_llm_manifest_path, resolve_llm_model_path
from .deterministic_summary import DETERMINISTIC_SUMMARY_MODEL, LEGACY_DETERMINISTIC_SUMMARY_MODEL, build_deterministic_summary
...
def summary_failure_code(exc: BaseException) -> str:
    text = f"{type(exc).__name__}: {exc}".lower()
    ...
    if any(token in text for token in ("invalid_json", "schema_validation", "summary_schema_invalid")):
...
class SummaryRepository:
    ...
    def mark_failed(self, job_id: str, meeting_id: str, error: str, message_id: str | None = None, error_code: str = "SUMMARY_FAILED") -> None:
        if error_code == "SUMMARY_FAILED":
            error_code = summary_failure_code(RuntimeError(error))
        with self._db.connection() as connection:
            with connection.transaction():
                connection.execute(
                    "UPDATE jobs SET status='FAILED',stage='FAILED',progress=0,error_message=%s,error_code=%s,lease_expires_at=NULL,last_heartbeat=NULL,updated_at=now() WHERE id=%s AND status <> 'CANCELLED'",
    ...
    def persist(
        ...
        *,
        ...
        with self._db.connection() as connection:
            ...
            existing_summary = connection.execute("SELECT id,status,model_name FROM summaries WHERE job_id=%s FOR UPDATE", (job_id,)).fetchone()
            summary_id = None
            if existing_summary is not None:
                existing_status = str(existing_summary[1])
                existing_model = str(existing_summary[2] or "")
                # A deterministic draft is intentionally replaceable by a
                # later Qwen retry. Normal terminal summaries remain
                # idempotent and are never regenerated by duplicate NATS
                # deliveries.
                replaceable_draft = existing_status == "NEEDS_REVIEW" and existing_model in {DETERMINISTIC_SUMMARY_MODEL, LEGACY_DETERMINISTIC_SUMMARY_MODEL}
                if existing_status == "READY" or (existing_status == "NEEDS_REVIEW" and not replaceable_draft):
                    ...
                    return True
                if existing_status not in {"FAILED", "NEEDS_REVIEW"}:
            ...
            if summary_status == "FAILED":
                ...
                # Leave the job retryable. The caller advances attempt and
                # releases the NATS inbox lease in one transaction.
                connection.execute("UPDATE jobs SET status='RUNNING',stage='RETRY_PENDING',progress=0,error_message=%s,error_code='LLM_ENHANCEMENT_PENDING',lease_expires_at=now()+interval '30 minutes',last_heartbeat=now(),updated_at=now() WHERE id=%s AND status <> 'CANCELLED'", ("Детерминированный черновик сохранён; ожидается Qwen", job_id))
    ...
    async def handle(self, payload: dict[str, Any]) -> None:
        ...
        try:
            ...
            async with self._gpu_lease:
                ...
                try:
                    ...
                    if callable(record_probe):
                        ...
                        record_probe,
                        "READY",
                        total_ms=(time.perf_counter() - generation_started_at) * 1000.0,
                        first_token_ms=getattr(client, "last_first_token_ms", None),
                    )
            ...
            if is_retryable_summary_error(exc):
                # Make a safe, visible draft before scheduling the retry. It
                # is derived only from canonical segments and never claims
                # that Qwen has answered. A failed fallback must not hide the
                # original infrastructure error in the job diagnostics.
                if segments:
                    try:
                        fallback = build_deterministic_summary(
                            segments,
                            profile=str(payload.get("summary_profile", payload.get("profile")) or ""),
                        )
                        if correlation_id:
                            fallback["correlation_id"] = correlation_id
                        await asyncio.to_thread(
                        ...
                        LOGGER.warning("deterministic summary fallback failed job=%s", job_id, exc_info=True)
...
async def run() -> None:
    ...
    model_attestation_key: tuple[Any, ...] | None = None
    ...
    def summary_capabilities() -> dict[str, Any]:
        nonlocal model_attestation_key, model_attestation
        model_path = str(resolve_llm_model_path())
        manifest_path = str(resolve_llm_manifest_path(Path(model_path)))
        llama_binary = os.getenv("LLM_SERVER_BINARY", os.getenv("LLAMA_RUNTIME_BINARY", "/opt/llama/llama-server"))
        ...
        if model_key != model_attestation_key:
            model_attestation_key = model_key
            model_attestation = attest_llm_model()
        ...
        manifest_valid = bool(manifest_valid and model_attestation.get("manifestValid", False))
        if not manifest_valid and validation_reason == "ready":
            validation_reason = str(model_attestation.get("reason") or "model_manifest_mismatch")
        return {
            ...
            "llmModelIdentity": model_attestation.get("revision") or os.getenv("LLM_MODEL_ALIAS", "qwen3-8b"),
            "llmRuntimeBinary": llama_binary,
            ...
            "llmLastProbe": shared_runtime.last_probe,
